...
# TODO
@app.post("/bookmarks/category")
async def add_bookmark_category(body: dict):
    async with httpx.AsyncClient() as client:
        try:
            data = {
                "title": body.get("title")
            }
            response = await client.post(f"{BOOKMARK_SERVICE_URL}/category", json=data)
            return response
        except Exception as e:
            return ResponseModel(
                response=str(e),
                message=f"An error occurred when creating bookmark category",
                statusCode=500,
                status=ResponseStatus.failed
            )

# TODO
@app.post("/bookmarks/category/add")
async def add_bookmark_to_category(body: BookmarkCategoryCreateDto):
    async with httpx.AsyncClient() as client:
        try:
            print(BOOKMARK_SERVICE_URL)
            response = await client.post(f"{BOOKMARK_SERVICE_URL}/category/add", json=body)
            print(response)
            return response
        except Exception as e:
            return ResponseModel(
